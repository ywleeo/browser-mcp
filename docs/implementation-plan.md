...
- 页面写操作：仅增加小红书/抖音点赞与收藏的站点级窄工具，其他写操作仍不在当前计划中。
...
| 5 | 小红书 adapter | 搜索、笔记详情、账号笔记、评论流、媒体下载及点赞/收藏 |
...
- `0.10.0` 增加 `xhs_like`、`xhs_collect`、`douyin_like`、`douyin_collect`。四个工具以
  `enabled` 表达期望状态，写操作进入平台独立 `*.mutate` 命名空间；状态一致不点击，状态
  不一致最多点击一次并验证，验证失败不重试。
