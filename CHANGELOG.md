...
- 回收不再把未回收的僵尸进程当成活着的 server。一个已经退出、但父进程尚未 `wait()` 的子进程会
  留在进程表里，`os.kill(pid, 0)` 对它依旧成功——而内核在它退出时就已经收回了监听套接字。
  结果是回收白等满超时、报告失败，`start()` 因此不再重试绑定，对着一个其实已经空出来的端口
  抛出"端口池耗尽"。存活判断现在读进程状态，`Z` 一律视为已退出。这个差异只在 Linux 上稳定复现
  （macOS 的 asyncio 收割得更快），本地测试全绿而 CI 挂掉，正是它。
