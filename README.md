...
- **登录态不泄露**：扩展不申请 `cookies` 权限，不返回、不持久化 Cookie——登录态只在你自己的 Chrome
  里被页面正常使用，从不经 MCP 传出。
- **记录与产物留本地**：连接、工具调用与媒体下载状态都保存在本地；下载落盘并记录 SHA-256，不对外上报。
- **完全合规**：不逆向内部接口、不绕过验证码、不窃取 Cookie、不批量抓取——行为就是一个普通登录用户
  在浏览。
- **边界说明**：你让 AI 读取的**页面内容**会作为工具结果返回给 AI 助手（这正是它的用途）；若该助手
  连接的是模型 API，那部分内容会进入模型服务。但**登录态与 Cookie 永不随之外泄**——这点与「把内容
  交给 AI」是两回事。
...
- **后台操作网页**：在共享登录态的后台 Chrome 窗口里返回截图 + 编号可操作元素，继续点击、滚动、输入、按键、下拉、**上传文件**；不切走你当前的页面。
- **知乎**：搜索、问题、回答、文章、邀请回答。
...
当前版本为 `0.15.0`，版本变更见 [CHANGELOG.md](CHANGELOG.md)。
...
- `<all_urls>`：用于打开调用方明确请求的公开 HTTP(S) 页面，并支持多个站点 adapter；不会
  主动遍历浏览历史。
- `debugger`：用于捕获页面请求响应，在评论流和视觉交互中发送可信浏览器输入事件，以及把本地
  文件直接写入网页的文件输入框（系统文件选择框不会弹出）。
...
| `browser_upload` | 网页操作 | 把本地文件直接写入网页的文件输入框，并返回上传后的页面状态。Chrome 自己读取路径，因此不会弹出系统文件选择框，也不需要操作它——**永远不要为了上传去点上传按钮**，那会弹出系统对话框，任何浏览器工具都关不掉也填不了。多数上传按钮把真正的 `input[type=file]` 藏了起来，这类元素不会出现在 `browser_snapshot` 中，所以先不传 `element_id` 直接调用：页面只有一个文件输入框时自动选中；有多个时报错会列出每个候选的序号、accept 限制与邻近文字，据此带 `index` 重试。当页面上还没有文件输入框（上传控件点击时才用 JS 创建），或想把查找范围限定到某个可见的上传按钮时，再传 `element_id`：此时会在拦截文件选择框的前提下点击该控件，系统对话框不会出现，Chrome 会直接交出那个刚被创建的输入框。路径必须是可读的普通文件，凭据目录与敏感 dotfile 会被拒绝。 |
